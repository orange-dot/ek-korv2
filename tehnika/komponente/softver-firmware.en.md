# Software and Firmware for EV Chargers
...
### 1.2 Multi-Processor Distribution

```
Function Distribution Across Processors:
...
+---------------------------------------------------------------+
| Task                         | RTOS  | Bare-metal | Recommend  |
+------------------------------+-------+------------+------------+
| PWM control loop (>10kHz)    |  X    |     OK     | Bare-metal |
| Current loop (<10us)         |  X    |     OK     | Bare-metal |
| Voltage loop (<100us)        |  ~    |     OK     | Bare-metal |
| Communication (CAN, etc.)    |  OK   |     ~      | RTOS       |
| State machine (ms level)     |  OK   |     OK     | Either     |
| HMI updates                  |  OK   |     X      | RTOS       |
| Logging/Diagnostics          |  OK   |     X      | RTOS       |
| OCPP/ISO 15118               |  OK   |     X      | RTOS       |
+---------------------------------------------------------------+
...
// Execution time: ~2-3 us @ 200 MHz
// Available time @ 50 kHz: 20 us
// CPU utilization for control: ~15%
```

### 2.3 Charging State Machine
...
1. Josev (Open Source):
   ...
   | - Actively maintained                       |
   | - Good documentation                        |
   | - Used with I2SE PLC modules                |
   |                                             |
   | Limitations:                                |
   | - Python (requires Linux SBC)               |
   | - Not for embedded MCU directly             |
   +---------------------------------------------+

2. OpenV2G (Siemens, older):
   +---------------------------------------------+
   ...
   | - Outdated, limited support                 |
   +---------------------------------------------+

3. Chargebyte (Commercial):
   +---------------------------------------------+
   | chargebyte.com                              |
   |                                             |
   | - Turnkey solution (HW + SW)                |
   | - ISO 15118-2, -20, DIN 70121               |
   | - Certified stack                           |
   | - Technical support                         |
   | - Price: EUR5,000-20,000 license            |
   +---------------------------------------------+

4. Switch EV (Commercial):
   +---------------------------------------------+
   | switch-ev.com                               |
   |                                             |
   | - Based on Josev                            |
...
1. Local Authorization List:
   - Predefined tokens in flash memory
   - OCPP: SendLocalList, GetLocalListVersion

2. Authorization Cache:
   - Cached tokens from CSMS
   - Timeout-based expiry

3. Offline Transactions:
   - Storing transactions in local memory
   - Synchronization when connection established
...
3. Web-based HMI:
   ...
   | Cost: EUR50-150 (tablet/panel PC)                             |
   +---------------------------------------------------------------+
...
+-- Oscilloscope with decode
```

## 9. Bill of Materials - Software
...
| Item                             | Price        | Note         |
+----------------------------------+--------------+--------------+
| ISO 15118 Stack License          | EUR5,000-20,000| One time   |
| (commercial, e.g. Chargebyte)    |              |              |
+----------------------------------+--------------+--------------+
| Alternative: Josev (open source) | EUR0         | Dev effort   |
| + I2SE hardware                  | EUR100/unit  | Per unit     |
+----------------------------------+--------------+--------------+
| OCPP Stack (open source)         | EUR0         | Dev effort   |
+----------------------------------+--------------+--------------+
| RTOS (FreeRTOS)                  | EUR0         | Open source  |
+----------------------------------+--------------+--------------+
| GUI Framework (LVGL)             | EUR0         | Open source  |
+----------------------------------+--------------+--------------+
| TLS Library (mbedTLS)            | EUR0         | Open source  |
+----------------------------------+--------------+--------------+
| IDE/Toolchain                    | EUR0-2,000   | Per seat/year|
+----------------------------------+--------------+--------------+
| Test Tools (Vector CANoe)        | EUR15,000+   | Per seat     |
+----------------------------------+--------------+--------------+
| Conformance Certification        | EUR10,000-30,000| Per product|
+---------------------------------------------------------------+
