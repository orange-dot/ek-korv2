...
/* Spin table defined in boot.S */
extern volatile uint64_t spin_table[3];
...
/**
 * @brief Entry point for secondary cores
 *
 * Called from boot.S after secondary core wakes from spin_table.
 * Stack is already set up by boot.S.
 */
...
int smp_start_core(uint32_t core_id, void (*entry)(uint32_t))
{
    ...
    /* Store entry function */
    g_secondary_entry = entry;

    /* Memory barrier to ensure entry is visible */
    __asm__ volatile("dmb sy" ::: "memory");

    /* Write entry point to spin table */
    spin_table[core_id - 1] = (uint64_t)(uintptr_t)secondary_core_entry;
