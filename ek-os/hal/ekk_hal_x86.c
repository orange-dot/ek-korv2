/**
 * @file ekk_hal_x86.c
 * @brief EK-KOR HAL Implementation for x86_64 (EK-OS)
 *
...
#include "../ekk/include/ekk/ekk_hal.h"
#include "../ekk/include/ekk/ekk_field.h"
...
#define FIELD_REGION_ADDR       0x300000UL
#define FIELD_REGION_SIZE       (64 * 1024)

/* Message queues for inter-core IPC */
#define MSG_QUEUE_ADDR          0x310000UL
...
static void calibrate_tsc(void) {
    ...
    /* Wait for PIT to count down (poll output bit) */
    /* Actually, for simplicity, just do a fixed delay loop calibration */
    /* This is approximate but sufficient for our purposes */

    /* Simple calibration: assume ~3 GHz and refine later */
    /* For accurate calibration, we'd need ACPI PM timer or HPET */
    g_tsc_freq_mhz = 3000;  /* Assume 3 GHz */
...
static core_queue_t *g_queues = (core_queue_t *)MSG_QUEUE_ADDR;
...
ekk_error_t ekk_hal_recv(ekk_module_id_t *sender_id,
    ...
    *msg_type = (ekk_msg_type_t)slot->msg_type;
    if (data && slot->len > 0) {
        ...
        memcpy(data, slot->data, copy_len);
    }
    *len = slot->len;
...
ekk_error_t ekk_hal_init(void) {
    ...
    serial_printf("[EK-KOR] HAL initialized for x86_64\n");
    serial_printf("[EK-KOR] Module ID: %u (APIC ID: %u)\n", g_module_id, g_module_id - 1);
    serial_printf("[EK-KOR] TSC frequency: ~%lu MHz\n", g_tsc_freq_mhz);
...
void ekk_hal_printf(const char *fmt, ...) {
    /* Use serial_printf from EK-OS drivers */
    char buf[256];
    va_list args;
    va_start(args, fmt);

    /* Simple implementation - forward to serial */
    serial_printf("[EKK] ");

    /* Manual format handling since we can't easily use vprintf in freestanding */
    const char *p = fmt;
    while (*p) {
        if (*p == '%') {
            p++;
