...
ekk_error_t ekk_field_publish(ekk_module_id_t module_id, const ekk_field_t *field)
{
    ...
    /* Get target slot (access inner field through coord wrapper) */
    ekk_coord_field_t *coord = &g_field_region->fields[module_id];
    ekk_field_t *target = &coord->field;
    ...
    for (int i = 0; i < EKK_FIELD_COUNT; i++) {
        target->components[i] = field->components[i];
    }
    target->timestamp = now;
    target->source = module_id;
    target->sequence++;

    /* Memory barrier after write */
    ekk_hal_memory_barrier();

    /* Set update flag in the bitmask array */
    uint32_t word_idx = module_id / 32;
    uint32_t bit = (1u << (module_id % 32));
    uint32_t state = ekk_hal_critical_enter();
    g_field_region->update_flags[word_idx] |= bit;
...
ekk_error_t ekk_field_sample(ekk_module_id_t target_id, ekk_field_t *field)
{
    ...
    /* Read from region (access inner field through coord wrapper) */
    const ekk_coord_field_t *coord = &g_field_region->fields[target_id];
    const ekk_field_t *source = &coord->field;

    /* Memory barrier before read */
    ekk_hal_memory_barrier();

    /* Check validity */
    if (source->source == EKK_INVALID_MODULE_ID) {
        return EKK_ERR_NOT_FOUND;
    }

    /* Check age */
    ekk_time_us_t age = now - source->timestamp;
    if (age > EKK_FIELD_MAX_AGE_US) {
        return EKK_ERR_FIELD_EXPIRED;
    }

    /* Copy field data */
    memcpy(field, source, sizeof(ekk_field_t));
...
uint32_t ekk_field_gc(ekk_time_us_t max_age_us)
{
    ...
    for (int i = 0; i < EKK_MAX_MODULES; i++) {
        ekk_field_t *field = &g_field_region->fields[i].field;

        if (field->source == EKK_INVALID_MODULE_ID) {
            continue;
        }

        ekk_time_us_t age = now - field->timestamp;
        if (age > max_age_us) {
            /* Mark as invalid */
