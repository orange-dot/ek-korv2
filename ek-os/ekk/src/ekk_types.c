...
ekk_fixed_t ekk_fixed_exp_decay(ekk_time_us_t elapsed_us, ekk_time_us_t tau_us)
{
     ...
     * t < tau:   1.0 - t/tau
     * t < 2*tau: 0.5 - (t-tau)/(2*tau)
     * t < 3*tau: 0.25 - (t-2*tau)/(4*tau)
     * t >= 3*tau: 0 (consider expired)
     */
    if (elapsed_us < tau_us) {
        /* f(t) = 1.0 - t/tau */
        ekk_fixed_t ratio = ekk_fixed_div(
            (ekk_fixed_t)(elapsed_us & 0xFFFFFFFF),
            (ekk_fixed_t)(tau_us & 0xFFFFFFFF)
        );
        return EKK_FIXED_ONE - ratio;
    }
    else if (elapsed_us < tau_us * 2) {
        /* f(t) = 0.5 - (t-tau)/(2*tau) */
        ekk_time_us_t t_offset = elapsed_us - tau_us;
        ekk_fixed_t ratio = ekk_fixed_div(
            (ekk_fixed_t)(t_offset & 0xFFFFFFFF),
            (ekk_fixed_t)((tau_us * 2) & 0xFFFFFFFF)
        );
        return EKK_FIXED_HALF - ratio;
    }
    else if (elapsed_us < tau_us * 3) {
        /* f(t) = 0.25 - (t-2*tau)/(4*tau) */
        ekk_time_us_t t_offset = elapsed_us - tau_us * 2;
        ekk_fixed_t quarter = EKK_FIXED_ONE >> 2;
        ekk_fixed_t ratio = ekk_fixed_div(
            (ekk_fixed_t)(t_offset & 0xFFFFFFFF),
            (ekk_fixed_t)((tau_us * 4) & 0xFFFFFFFF)
        );
        return quarter - ratio;
    }

    /* Expired: decay factor is effectively zero */
    return 0;
}
